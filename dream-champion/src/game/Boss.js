...
export class Boss extends Enemy {
  ...
  fixedUpdate(step, player, enemies) {
    ...
    if (this.weakCd > 0) this.weakCd -= step;
...
// Phase 1 teaches the moves, phase 3 stops giving you time to think. The old flat rnd(1.4,2.4) gap
// meant a 21 s fight showed four attacks out of a kit of seven -- most of the fight was never seen.
function moveGap(e) { return ([1.45, 1.0, 0.7][e.phase - 1] ?? 0.7) * (e.enraged ? 0.7 : 1) * rnd(0.8, 1.3); }
// A boss that whiffs a named move is WIDE OPEN. Dodging is what earns the damage window, so the fight
// reads as a rhythm to learn instead of a health bar to grind.
// Rate-limited on purpose: a bait window is a REWARD, not the boss's resting state. Without the
// cooldown a player who dodges everything keeps it permanently open and the whole fight ends in 5 s.
function openWeak(e, g, t = 0.9) { if ((e.weakCd || 0) > 0 || e.weakOpen) return; e.weakOpen = true; e.weakT = t; e.weakCd = 5.5; g.hud.stamp('WIDE OPEN - HIT IT', 'gold'); g.audio.play('sfx_tick', { vol: 0.7 }); }
function bossBrain(moves) {
  return function (e, step, player, dist, nx, nz, g) {
    if (e.stagger > 0) return;
    if (e.move) { const done = e.move.run(e, step, player, dist, nx, nz, g); if (done) { e.lastMove = e.move; e.move = null; e.moveCd = moveGap(e); e.state = 'move'; } return; }
    e.moveCd -= step;
    if (e.moveCd <= 0) {
      let pool = moves.filter(m => m.phases.includes(e.phase) && (!m.can || m.can(e, dist, g)));
      if (pool.length > 1 && e.lastMove) { const p2 = pool.filter(m => m !== e.lastMove); if (p2.length) pool = p2; }
      const m = pick(pool); if (m) { e.move = m; e.mt = 0; e.ms = 0; m.start(e, player, dist, nx, nz, g); return; }
    }
...
const lunge = (name, windup, len, speed, dmg, clip) => ({ name, phases: [1, 2, 3], can: (e, d) => d > 3 && d < len, start(e, p, d, nx, nz, g) { e.state = 'windup'; e.windup = windup; e.vx = e.vz = 0; e.lx = nx; e.lz = nz; e.decal = lineDecal(e, g, nx, nz, len, windup); g.audio.play('sfx_' + e.def.voice + '_windup', { x: e.x, z: e.z, vol: 1 }); if (e.anim) e.anim.shot(clip || e.def.clips.stomp, null, 0.05, { timeScale: 1.43 / windup }); e.ms = 0; e.hitDone = false; },
  run(e, step, p, d, nx, nz, g) { e.mt += step; if (e.ms === 0) { e.windup -= step; e.faceTo(e.lx, e.lz, step, 10); if (e.windup <= 0) { e.ms = 1; e.mt = 0; killDecal(e, g); e.state = 'lunge'; g.audio.play('sfx_' + e.def.voice + '_roar', { x: e.x, z: e.z, vol: 0.9, rate: 1.1 }); if (e.anim) e.anim.play(e.def.clips.rush || e.def.clips.move, 0.1); } return false; }
    ...
    if (e.mt > dur) { e.vx = e.vz = 0; e.state = 'move'; if (!e.hitDone) openWeak(e, g, 1.1); return true; } return false; } });
const cone = (name, phases, windup, range, angle, dmg, clip) => ({ name, phases, can: (e, d) => d < range + 1.5, start(e, p, d, nx, nz, g) { e.state = 'windup'; e.windup = windup; e.vx = e.vz = 0; e.hitDone = false; e.decal = g.decals.add(e.x + nx * range * 0.5, e.z + nz * range * 0.5, { type: D.CONE, size: range * 2.1, rot: -Math.atan2(nx, nz) + Math.PI, color: _c.set(RED), life: windup + 0.15, alpha: 0.6, fadeIn: 0.2 }); if (e.anim) e.anim.shot(clip, null, 0.05, { timeScale: 1.87 / (windup + 0.4) }); g.audio.play('sfx_' + e.def.voice + '_windup', { x: e.x, z: e.z, vol: 1 }); },
  run(e, step, p, d, nx, nz, g) { e.mt += step; if (e.ms === 0) { e.windup -= step; e.faceTo(nx, nz, step, 4); if (e.windup <= 0) { e.ms = 1; e.mt = 0; killDecal(e, g); const ang = Math.abs(angDiff(e.face, Math.atan2(nx, nz))); e.hitDone = (d < range + p.r && ang < angle * Math.PI / 360) ? p.hurt(dmg, nx, nz, e.key) : false; g.shake(0.45); g.audio.play('sfx_' + e.def.voice + '_swing', { x: e.x, z: e.z, vol: 1 }); e.ctx.particlesAlpha.burst(e.x + nx * 2, 0.3, e.z + nz * 2, { n: 16, type: P.SMOKE, color: _c.set(0x1a1410), speed: 3, life: 0.8, size: 0.6, sizeEnd: 1.8 }); } return false; } if (e.mt > 0.8) { if (!e.hitDone) openWeak(e, g, 0.9); return true; } return false; } });
const slam = (name, phases, windup, radius, dmg, leap) => ({ name, phases, can: (e, d) => d < 14, start(e, p, d, nx, nz, g) { e.state = 'windup'; e.windup = windup; e.tx = p.x; e.tz = p.z; e.sx = e.x; e.sz = e.z; e.vx = e.vz = 0; e.decal = g.decals.add(e.tx, e.tz, { type: D.DISC, size: radius * 2, color: _c.set(RED), life: windup + 0.15, alpha: 0.5, fadeIn: 0.1, tele: true }); if (e.anim) e.anim.shot(leap ? e.def.clips.leap : e.def.clips.slam, null, 0.05, { timeScale: (leap ? 2.37 : 1.87) / (windup + 0.3) }); g.audio.play('sfx_' + e.def.voice + '_roar', { x: e.x, z: e.z, vol: 0.8, rate: 1.2 }); },
  run(e, step, p, d, nx, nz, g) { e.mt += step; if (e.ms === 0) { e.windup -= step; const u = clamp(1 - e.windup / windup, 0, 1); if (leap) { e.x = e.sx + (e.tx - e.sx) * u; e.z = e.sz + (e.tz - e.sz) * u; e.inner.position.y = -e.proto.minY * e.proto.baseScale + Math.sin(u * Math.PI) * 6; e.noHit = u < 0.85; } e.faceTo(e.tx - e.x, e.tz - e.z, step, 6);
      if (e.windup <= 0) { e.ms = 1; e.mt = 0; killDecal(e, g); e.noHit = false; e.inner.position.y = -e.proto.minY * e.proto.baseScale; const cx = leap ? e.x : e.x + Math.sin(e.face) * e.r, cz = leap ? e.z : e.z + Math.cos(e.face) * e.r; e.hitDone = hitPlayerRadius(e, p, cx, cz, radius, dmg, g); if (e.hitDone) { p.vx += nx * 8; p.vz += nz * 8; } g.shake(0.85); g.loop.stopTime(60); g.audio.play('sfx_boss_slam', { vol: 1 }); g.decals.add(cx, cz, { type: D.CRACK, size: radius * 2.4, color: _c.set(0x000000), life: 30, alpha: 0.8 }); e.ctx.particlesAdd.one(cx, 0.3, cz, { type: P.RING, color: _c.set(0xffb070), life: 0.5, size: 1, sizeEnd: radius * 3, vx: 0, vy: 0, vz: 0 }); e.ctx.particlesAlpha.burst(cx, 0.2, cz, { n: 40, type: P.CHUNK, color: _c.set(0x3a2a1a), speed: 7, life: 1, size: 0.15, gravity: 12, up: 4 }); e.ctx.particlesAlpha.burst(cx, 0.3, cz, { n: 20, type: P.SMOKE, color: _c.set(0x201810), speed: 3, life: 1.2, size: 0.8, sizeEnd: 2.4 }); } return false; } if (e.mt > 0.9) { if (!e.hitDone) openWeak(e, g, 1.2); return true; } return false; } });
...
const summon = (name, phases, type, count, kindOverride, banner) => ({ name, phases, can: (e, d, g) => g.director.addCount() + count <= (e.phase >= 3 ? 8 : e.phase === 2 ? 6 : 4), start(e, p, d, nx, nz, g) { e.state = 'windup'; e.windup = 1.4; e.vx = e.vz = 0; if (e.anim) e.anim.shot(e.def.clips.scream, null, 0.05, { timeScale: 2 }); g.audio.play('sfx_' + e.def.voice + '_call', { x: e.x, z: e.z, vol: 1 }); if (banner) g.hud.banner(banner, 'small'); },
...
// Shockwave: one ring, one roll. The most readable "move NOW" in the game -- and whiffing it opens
// the boss up, so a kid who learns the timing is rewarded twice.
const shockwave = (name, phases, windup, dmg, speed = 9, maxR = 17) => ({ name, phases, can: () => true,
  start(e, p, d, nx, nz, g) {
    e.state = 'windup'; e.windup = windup; e.vx = e.vz = 0; e.shockR = 0; e.shockHit = false; e.sx = e.x; e.sz = e.z;
    e.decal = g.decals.add(e.x, e.z, { type: D.RING, size: 7, color: _c.set(RED), life: windup + 0.2, alpha: 0.6, fadeIn: 0.12 });
    const clips = e.def.clips; if (e.anim && clips) e.anim.shot(clips.stomp || clips.slam || clips.attack, null, 0.05, { timeScale: 1.87 / (windup + 0.2) });
    g.audio.play('sfx_' + e.def.voice + '_windup', { x: e.x, z: e.z, vol: 1 });
  },
  run(e, step, p, d, nx, nz, g) {
    e.mt += step;
    if (e.ms === 0) {
      e.windup -= step; e.faceTo(nx, nz, step, 3);
      if (e.windup <= 0) {
        e.ms = 1; e.mt = 0; killDecal(e, g); e.sx = e.x; e.sz = e.z; g.shake(0.75); g.loop.stopTime(50); g.audio.play('sfx_boss_slam', { vol: 1 });
        e.ctx.particlesAdd.one(e.x, 0.3, e.z, { type: P.RING, color: _c.set(RED), life: maxR / speed, size: 1.5, sizeEnd: maxR * 2, vx: 0, vy: 0, vz: 0 });
      } return false;
    }
    e.shockR += speed * step;
    if (!e.shockHit) { const dd = Math.hypot(p.x - e.sx, p.z - e.sz); if (Math.abs(dd - e.shockR) < 1.0) { const dl = dd || 1; e.shockHit = p.hurt(dmg, (p.x - e.sx) / dl, (p.z - e.sz) / dl, e.key); } }
    if (e.shockR > maxR) { if (!e.shockHit) openWeak(e, g, 1.3); return true; } return false;
  } });
...
const stagMoves = [
  ...
  summon('Thorn Chorus', [2, 3], 'thorn', 2, 'treeline', null),
  shockwave('Antler Quake', [3], 1.05, 28),
];
// ---------- THE ROT KING ----------
const kingMoves = [
  ...
  shockwave('Rot Pulse', [2, 3], 1.15, 26, 8.5),
];
// ---------- MEGALODON ----------
const megMoves = [
  ...
  shockwave('Tidal Roar', [3], 1.2, 26, 11, 20),
