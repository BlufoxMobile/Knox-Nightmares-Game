...
export class Director {
  constructor(ctx) { this.ctx = ctx; this.game = ctx.game; this.enemies = []; this.pools = {}; this.protos = {}; this.tokens = { melee: 2, ranged: 1 }; this.held = { melee: [], ranged: [] }; this.pickups = []; this.puddles = []; }
  setup(worldKey) {
  ...
  spawnEnemy(type, kind) {
    const e = this.get(type); const def = ENEMIES[type]; const sp = kind === 'ground' ? { x: rnd(-8, 8), z: rnd(-8, 8), kind: 'ground' } : this.spawnPoint(kind || (this.worldKey === 'grave' ? 'grave' : this.worldKey === 'sea' ? 'water' : 'treeline'));
  ...
  fixedUpdate(step) {
    ...
    if (this.state === 'prewave' && this.stateT <= 0) this.nextWave();
      ...
      if (this.queue.length && this.spawnT <= 0 && aliveN < this.cap) {
        ...
        this.spawnT = this.spawnGap * rnd(0.7, 1.2) * (this.waveIdx === 0 ? 1 : fill < 0.35 ? 0.28 : fill < 0.7 ? 0.6 : 1);
      }
      if (!this.queue.length && aliveN === 0) { this.state = 'between'; this.stateT = this.waveIdx === this.waves.length - 1 ? 1.0 : 6.5; g.hud.banner(COPY.banners.clear, 'small'); g.audio.play('sfx_clear', { vol: 0.9 }); g.audio.setMusicMix({ bed: 1, combat: 0.2 }, 2); if (pl.hp < 50) g.spawnPickup(pl.x + rnd(-3, 3), pl.z + rnd(-3, 3), 'heart'); }
    }
    ...
    // pickups
    for (let i = this.pickups.length - 1; i >= 0; i--) { const p = this.pickups[i]; p.t -= step; const dx = pl.x - p.x, dz = pl.z - p.z; const d = Math.hypot(dx, dz); if (d < 2.5 && pl.alive) { p.x += dx / d * 12 * step; p.z += dz / d * 12 * step; } p.obj.position.set(p.x, 0.6 + Math.sin(g.time * 3 + p.x) * 0.15, p.z); p.obj.rotation.y += step * 2; if (d < 0.9 && pl.alive) { g.collectPickup(p); this.ctx.scene.remove(p.obj); this.pickups.splice(i, 1); } else if (p.t <= 0) { this.ctx.scene.remove(p.obj); this.pickups.splice(i, 1); } }
    // acid puddles
    for (let i = this.puddles.length - 1; i >= 0; i--) { const p = this.puddles[i]; p.t -= step; if (pl.alive && Math.hypot(pl.x - p.x, pl.z - p.z) < p.r) { p.tick -= step; if (p.tick <= 0) { p.tick = 0.5; pl.hurtSoft(2.5); } } if (p.t <= 0) this.puddles.splice(i, 1); }
  }
  ...
  onWake() { // player woke: keep boss phase, reset wave enemies
    ...
    if (this.state === 'boss' && this.boss) { const b = this.boss; b.stagger = 0; b.move = null; b.state = 'move'; b.moveCd = 3; b.x = this.game.world.env.bossSpawn?.x || 0; b.z = this.game.world.env.bossSpawn?.z || -12; b.hp = Math.max(b.hp, b.maxhp * (b.phase === 3 ? 0.33 : b.phase === 2 ? 0.66 : 1) * 0.999); b.noHit = false; }
